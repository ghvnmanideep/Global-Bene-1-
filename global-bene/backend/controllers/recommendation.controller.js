...
exports.getRecommendedPostsForUser = async (req, res) => {
  try {
    ...
    if (tagArray.length > 0 || topicArray.length > 0) {
      // Build a query that includes user's preferred tags and topics
      ...
      posts = await Post.find(personalizedQuery)
        .populate('author', 'username')
        .populate('community', 'name displayName')
    ...
    const paginatedPosts = uniquePosts.slice(parseInt(offset), parseInt(offset) + parseInt(limit));
