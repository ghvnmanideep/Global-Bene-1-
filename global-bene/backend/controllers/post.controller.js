...
exports.getPostById = async (req, res) => {
  try {
    ...
    // Increment total views for the viewing user
    if (req.user && req.user.id) {
      const viewingUser = await User.findById(req.user.id);
      if (viewingUser) {
        viewingUser.totalViews = (viewingUser.totalViews || 0) + 1;
...
exports.votePost = async (req, res) => {
  try {
    ...
    if (post.votes && Array.isArray(post.votes)) {
      ...
      if (voteType === 'upvote') {
        // Increment totalLikesGiven for the user who is voting
        const votingUser = await User.findById(userId);
        if (votingUser) {
          votingUser.totalLikesGiven = (votingUser.totalLikesGiven || 0) + 1;
          await votingUser.save();
        }
        post.upvotes.push(userId);
        // Increment totalLikesReceived for the post author
...
exports.toggleSavePost = async (req, res) => {
  try {
    ...
    if (isSaved) {
      ...
      // Increment totalSaves for the user who is saving
      const savingUser = await User.findById(userId);
      if (savingUser) {
        savingUser.totalSaves = (savingUser.totalSaves || 0) + 1;
